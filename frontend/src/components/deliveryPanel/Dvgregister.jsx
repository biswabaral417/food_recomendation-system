...
export default function DvgRegister() {
    ...
    const PostData_Register_DVG = async (e) => {
        e.preventDefault()
        ...
        if ((document.getElementById('flexCheckChecked')).checked) {
            ...
            console.log({ rUserData })
            const res = await fetch("/api/dvg/register", {
                method: "POST",
                headers: {
    ...
    return (
        ...
        <div className='p-5 bg-light' style={styletxt}>
            <h1 className='text-center text-black fs-3'>Apply for DeliveryGuy</h1>
            ...
            <div>
                <form method='post'>
                    ...
                    <button className="btn btn-primary" onClick={PostData_Register_DVG} >Apply Application</button>
                </form>
            </div>
